...
public class ProgressBarCircularIndeterminate extends CustomView {
	...
	protected void setAttributes(AttributeSet attrs) {
		setMinimumHeight(ScreenUitlity.dip2px(getContext(), 32));
		setMinimumWidth(ScreenUitlity.dip2px(getContext(), 32));
		...
		setMinimumHeight(ScreenUitlity.dip2px(getContext(), 3));
	}
		...
		if (radius1 < getWidth() / 2) {
			...
			if (cont >= 50) {
				...
				radius2 = (radius2 >= getWidth() / 2
						- ScreenUitlity.dip2px(getContext(), 4)) ? (float) getWidth()
						/ 2 - ScreenUitlity.dip2px(getContext(), 4) : radius2 + 1;
			}
			temp.drawCircle(getWidth() / 2, getHeight() / 2, radius2,
					transparentPaint);
			canvas.drawBitmap(bitmap, 0, 0, new Paint());
			if (radius2 >= getWidth() / 2 - ScreenUitlity.dip2px(getContext(), 4))
				cont++;
	...
	private void drawSecondAnimation(Canvas canvas) {
		...
		Bitmap bitmap = Bitmap.createBitmap(canvas.getWidth(),
				canvas.getHeight(), Bitmap.Config.ARGB_8888);
		Canvas temp = new Canvas(bitmap);
		temp.setDrawFilter(new PaintFlagsDrawFilter(0, Paint.ANTI_ALIAS_FLAG
				| Paint.FILTER_BITMAP_FLAG));
		Paint paint = new Paint();
		paint.setAntiAlias(true);
		...
		temp.drawCircle(getWidth() / 2, getHeight() / 2, (getWidth() / 2)
				- ScreenUitlity.dip2px(getContext(), 4), transparentPaint);
